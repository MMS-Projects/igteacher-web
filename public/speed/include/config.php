...
define('URL','http://www.igteacher.com/speed/'); //IMPORTANT please enter with latest backslash
...
define('PASS','igtadmin'); //The password for the MySQL database
...
function text($id) {
	$query = mysql_query("SELECT text FROM speed_texts WHERE id = ".$id);
	while($lang = mysql_fetch_object($query)){
		$return = $lang->text;
	}
	return $return;
...
function game($game) {
	...
	if($game == 'bountyhounds'){
	$return = 'Bounty Hounds Online';
	}
	if($game == 'forge'){
